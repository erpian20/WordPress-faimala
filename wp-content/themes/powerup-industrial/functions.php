...
function powerup_theme_get_currency_switcher_items() {
  ...
  if ( class_exists( 'WOOCS' ) ) {
    ...
    if ( is_object( $WOOCS ) && method_exists( $WOOCS, 'get_currencies' ) ) {
      $currencies = (array) $WOOCS->get_currencies();
      $current    = '';

      if ( isset( $WOOCS->current_currency ) && is_string( $WOOCS->current_currency ) ) {
        $current = strtoupper( $WOOCS->current_currency );
      }

      $base_url = remove_query_arg( array( 'currency' ) );
...
function powerup_theme_get_health_check_data() {
  ...
  $robots_response = wp_remote_get( $robots_url, array( 'timeout' => 6, 'redirection' => 2 ) );
  if ( ! is_wp_error( $robots_response ) ) {
    $robots_code = (int) wp_remote_retrieve_response_code( $robots_response );
    $robots_ok   = ( 200 === $robots_code );
  }

  $sitemap_response = wp_remote_get( $sitemap_url, array( 'timeout' => 6, 'redirection' => 2 ) );
  if ( ! is_wp_error( $sitemap_response ) ) {
    $sitemap_code = (int) wp_remote_retrieve_response_code( $sitemap_response );
    $sitemap_ok   = ( 200 === $sitemap_code );
  }

  if ( defined( 'WP_ENVIRONMENT_TYPE' ) && 'local' === WP_ENVIRONMENT_TYPE ) {
    $warnings[] = __( 'WP_ENVIRONMENT_TYPE is set to local. This should be changed before production launch.', 'powerup-theme' );
  }

  $support_email = isset( $config['contact']['support_email'] ) ? (string) $config['contact']['support_email'] : '';
  if ( '' === $support_email || false !== stripos( $support_email, 'example.com' ) ) {
    $warnings[] = __( 'Contact support email is still a placeholder value.', 'powerup-theme' );
  }

  $generated_image_base_url = isset( $config['media']['generated_image_base_url'] ) ? (string) $config['media']['generated_image_base_url'] : '';
  if ( '' !== $generated_image_base_url && false !== strpos( $generated_image_base_url, 'trae-api-cn.mchost.guru' ) ) {
    $warnings[] = __( 'Generated image base URL still points to the temporary trae API endpoint.', 'powerup-theme' );
  }

  $seo_home_desc = isset( $config['seo']['home_description'] ) ? trim( (string) $config['seo']['home_description'] ) : '';
  $seo_shop_desc = isset( $config['seo']['shop_description'] ) ? trim( (string) $config['seo']['shop_description'] ) : '';
  $seo_blog_desc = isset( $config['seo']['blog_description'] ) ? trim( (string) $config['seo']['blog_description'] ) : '';

  if ( '' === $seo_home_desc ) {
    $warnings[] = __( 'SEO home description is empty. Add a clear homepage description in PowerUp Config.', 'powerup-theme' );
  }
  if ( '' === $seo_shop_desc ) {
    $warnings[] = __( 'SEO shop description is empty. Add a product catalog description in PowerUp Config.', 'powerup-theme' );
  }
  if ( '' === $seo_blog_desc ) {
    $warnings[] = __( 'SEO blog description is empty. Add a blog summary in PowerUp Config.', 'powerup-theme' );
  }

  if ( defined( 'DB_USER' ) && 'root' === DB_USER ) {
    $warnings[] = __( 'Database user is root in wp-config.php. Replace with production credentials before launch.', 'powerup-theme' );
  }

  if ( ! $robots_ok ) {
    $warnings[] = __( 'robots.txt is not reachable with status 200. Check crawl settings before launch.', 'powerup-theme' );
  }
...
function powerup_theme_render_amazon_like_gallery() {
  ...
  foreach ( $items as $index => $item ) {
    ...
    echo '<button type="button" class="powerup-amz-thumb' . esc_attr( $is_active ) . '" data-image="' . esc_url( $item['full'] ) . '" data-zoom="' . esc_url( $item['zoom'] ) . '" data-alt="' . esc_attr( $item['alt'] ) . '" aria-label="Switch product image">';
    echo '<img src="' . esc_url( $item['thumb'] ) . '" alt="' . esc_attr( $item['alt'] ) . '" loading="lazy" />';
    echo '</button>';
  }

  echo '</div>';
  echo '<div class="powerup-amz-main" data-zoom-image="' . esc_url( $first['zoom'] ) . '">';
  echo '<img class="powerup-amz-main-image" src="' . esc_url( $first['full'] ) . '" alt="' . esc_attr( $first['alt'] ) . '" />';
...
function powerup_theme_render_pdp_mix_match_note() {
  if ( ! function_exists( 'is_product' ) || ! is_product() ) {
    return;
  }

  echo '<p class="powerup-pdp-mix-match-note">Mix & Match Available</p>';
}
add_action( 'woocommerce_single_product_summary', 'powerup_theme_render_pdp_mix_match_note', 29 );

function powerup_theme_get_pdp_about_item_points( $product ) {
  $points = array();
  $raw    = get_post_meta( $product->get_id(), '_powerup_about_item_points', true );
  ...
  return array(
    __( 'Brushless motor: cordless chainsaw has a 2.56 hp engine with 12,000 RPM and chain speed up to 10 m/s for efficient cutting.', 'powerup-theme' ),
    __( '2*4000mAh battery: equipped with two 4.0Ah batteries plus overload and temperature protection for longer runtime.', 'powerup-theme' ),
    __( 'Multiple safety protection: includes safety hand guard and double-button start to reduce accidental activation risk.', 'powerup-theme' ),
    __( 'Automatic oiling and tool-less adjustment: 0.15 qt (140 ml) oil tank, auto lubrication, and easy tension dial for smoother cuts.', 'powerup-theme' ),
  );
}

function powerup_theme_render_pdp_about_this_item() {
  if ( ! function_exists( 'is_product' ) || ! is_product() ) {
    return;
  }
  ...
  $points = powerup_theme_get_pdp_about_item_points( $product );

  echo '<section class="powerup-pdp-about-item">';
  echo '<h2>' . esc_html__( 'About this item', 'powerup-theme' ) . '</h2>';
  echo '<ul>';
...
add_action( 'woocommerce_after_single_product_summary', 'powerup_theme_render_pdp_about_this_item', 6 );
...
function powerup_theme_render_pdp_call_me_back() {
  if ( ! function_exists( 'is_product' ) || ! is_product() ) {
    return;
  }

  global $product;
  $callback_status = isset( $_GET['pdp_callback'] ) ? sanitize_key( wp_unslash( (string) $_GET['pdp_callback'] ) ) : '';

  echo '<section class="powerup-pdp-callback">';
  echo '<h3>' . esc_html__( 'You Still Have a Question?', 'powerup-theme' ) . '</h3>';
  echo '<p>' . esc_html__( 'Leave your mobile phone number and we will call you back.', 'powerup-theme' ) . '</p>';
  if ( 'success' === $callback_status ) {
    echo '<div class="powerup-pdp-callback-notice is-success"><p>' . esc_html__( 'Your callback request has been sent.', 'powerup-theme' ) . '</p></div>';
  } elseif ( 'missing' === $callback_status || 'invalid' === $callback_status || 'failed' === $callback_status ) {
    echo '<div class="powerup-pdp-callback-notice is-error"><p>' . esc_html__( 'The callback request could not be submitted. Please try again.', 'powerup-theme' ) . '</p></div>';
  }
  echo '<form class="powerup-pdp-callback-form" action="' . esc_url( admin_url( 'admin-post.php' ) ) . '" method="post">';
  echo '<input type="hidden" name="action" value="powerup_pdp_callback_submit">';
  echo '<input type="hidden" name="powerup_pdp_callback_product_id" value="' . esc_attr( $product instanceof WC_Product ? (string) $product->get_id() : '0' ) . '">';
  wp_nonce_field( 'powerup_pdp_callback_submit', 'powerup_pdp_callback_nonce' );
  echo '<input type="tel" name="powerup_pdp_callback_phone" placeholder="' . esc_attr__( 'Your phone number', 'powerup-theme' ) . '" aria-label="' . esc_attr__( 'Your phone number', 'powerup-theme' ) . '" required>';
  echo '<button type="submit">' . esc_html__( 'CALL ME BACK', 'powerup-theme' ) . '</button>';
  echo '</form>';
  echo '</section>';
}
add_action( 'woocommerce_after_single_product_summary', 'powerup_theme_render_pdp_call_me_back', 24 );

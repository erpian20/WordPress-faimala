...
add_action( 'init', 'powerup_seed_launch_reviews', 90 );
...
function powerup_seed_launch_reviews() {
	if ( ! class_exists( 'WC_Product_Simple' ) || ! function_exists( 'wc_get_product_id_by_sku' ) ) {
		return;
	}
	...
	foreach ( powerup_seed_review_data() as $review ) {
		$product_id = wc_get_product_id_by_sku( (string) $review['asin'] );
		if ( ! $product_id ) {
			continue;
		}

		$comment_id = powerup_seed_upsert_product_review( (int) $product_id, $review );
		if ( $comment_id ) {
			$inserted[] = array(
				'asin'       => (string) $review['asin'],
				'comment_id' => $comment_id,
			);
		}
	}

	powerup_seed_sync_review_stats_for_seeded_products();
	powerup_seed_clear_page_cache();

	update_option( 'powerup_review_seed_version', $review_seed_version, false );
	update_option(
		'powerup_review_seed_last_result',
		array(
			'version'  => $review_seed_version,
			'inserted' => $inserted,
			'run_time' => current_time( 'mysql' ),
		),
		false
...
function powerup_seed_upsert_product_review( $product_id, array $review ) {
	global $wpdb;
...
function powerup_seed_sync_review_stats_for_seeded_products() {
	foreach ( powerup_seed_product_data() as $item ) {
...
function powerup_seed_review_data() {
	return array(
		array(
